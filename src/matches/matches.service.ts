...
export class MatchesService {
    ...
    async create(dto: CreateMatchDto) {
        ...
        if (user) {
            const likeItem = await this.getOneMatchByUsersId(dto.user_id_one, dto.user_id_two)
            if (likeItem) {
                throw new HttpException('Мэтч уже создан', HttpStatus.BAD_REQUEST);
            }
            const like = await this.matchesRepository.create(dto)
            return like
        }
        throw new HttpException('Невозможно создать мэтч', HttpStatus.BAD_REQUEST);
    }

    async delete(dto: CreateMatchDto) {
        // const user = this.authService.currentUser()
        // if (user) {
        //     const likeItem = await this.getOneMatchByUsersId(dto.user_id_one, dto.user_id_two)
        //     if (likeItem) {
    ...
    async getMatches() {
        const user = this.authService.currentUser()
        if (user && user.id) {
            const matches = await this.matchesRepository.findAll({where: {
                [Op.or]: [
                    {
                        user_id_one: user.id,
                    },
    ...
    async getOneMatchByUsersId(user_id_one: number, user_id_two: number) {
        const match = await this.matchesRepository.findOne({where: {
            [Op.or]: [
                {
