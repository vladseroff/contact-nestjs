...
export class LikesService {
    ...
    async like(dto: CreateLikeDto) {
        ...
        if (user && user.id !== dto.user_id) {
            ...
            if (likeItem) {
                throw new HttpException('Лайк уже поставлен :)', HttpStatus.BAD_REQUEST);
            }
        ...
        throw new HttpException('Невозможно поставить лайк :(', HttpStatus.BAD_REQUEST);
    }
    ...
    async dislike(dto: CreateLikeDto) {
        ...
        if (user && user.id !== dto.user_id) {
            ...
            throw new HttpException('Лайк и не стоял, даже не пришлось дизлайкать :)', HttpStatus.BAD_REQUEST);
        }
        throw new HttpException('Невозможно убрать лайк :(', HttpStatus.BAD_REQUEST);
    }

    async getLikes() {
        const user = this.authService.currentUser()
        if (user && user.id) {
            const likes = await this.likesRepository.findAll({where: {user_id: user.id}})
            return likes
        }
